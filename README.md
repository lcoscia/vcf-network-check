# VCF 9.1 Network Planner — v1.20.0
...
| Version | Date | Notes |
|---|---|---|
| v1.20.0 | Jul 2026 | Two IP-entry improvements, designed from the official Broadcom "VCF 9.1 Planning and Preparation Workbook". Editable IP range (Range Start / Range End): in the VLAN Design tab, the "Range Start" field only showed a read-only computed range end (= start + required IPs − 1) — not matching the official workbook's convention where start AND end of range are two independent, freely-entered fields (e.g. VCF Management Services Runtime, VCF Automation, VKS Supervisor, SSP Node/Service Pool...). "Range End" is now a second editable field on every VLAN row; a "✨ Suggest" button proposes a starting value (same computation as before), freely editable afterward. The appliance-address "Auto-fill" button now consumes the actually-entered range (Range Start → Range End) instead of the app-computed required-IP count, with a new non-blocking warning if the entered range is smaller than the block's appliance count. 30-IP reservation for VCF Management Services: new checkbox (VCF Management Services block, VCF 9.1) mirroring the official Broadcom recommendation — "Minimum: 12 IP addresses are required for deployment. Recommended: 30 IP addresses can be added and are reserved for new VCF management services components or scaling out existing components." When checked, it sizes the VCF Services Runtime block directly as /27 (30 IPs) instead of /28 (12 IPs, current default minimum) |
| v1.19.0 | Jul 2026 | Substantial architectural fix to the VCF Management Dedicated VLAN Network Model (VCF 9.1). Official Broadcom documentation (requirement VCF-MGMT-DV-NET-REQD-001) mandates that VCF Management Services (Fleet/Instance/Runtime), VCF Operations, VCF Operations for Networks, VCF Automation and the License Server ALL be placed together on one single common dedicated VLAN, from Day 0, deployed automatically by the VCF Installer, with no API step. The app incorrectly defaulted VCF Automation to the Management VM Network under this model (a wrong behavior introduced as far back as v1.13.0), and the checkbox added in v1.18.0 made things worse by presenting this placement as a Day-2 option via the fleet lifecycle API — when no such option is documented by Broadcom. That checkbox has been removed: VCF Automation now automatically joins the same dedicated VLAN as Fleet/Operations/Networks/License Server under this model, as documented. Bug fixed along the way: the License Server was being double-counted in IP sizing (once in the Management VM Network VLAN, once in the dedicated VLAN where it is actually placed) — fixed, including an identical bug found in the NSX Overlay Segment model. Safety net: new informational (non-blocking) validation rule flagging when a project imported from an older JSON export still carries a legacy per-component "dedicated VLAN" flag — no effect under current models, but still honored for backward compatibility. No change affects the "NSX Overlay Segment" model (already correct) or the "Shared VLAN" model (already correct) |
| v1.18.0 | Jul 2026 | Two UI/usability fixes. Management Domain — host count: the field never accepted fewer than 3 hosts, even with NFS/VMFS storage where 2 hosts are officially supported under the "Simple" deployment model (Broadcom TechDocs VCF 9.1, Single-Rack vSphere Cluster Model). Fixed: the minimum is now dynamic (2 for NFS/VMFS, 3 for vSAN ESA/OSA), and the corresponding blocking validation rule (`core/validation.js`) was updated accordingly so it no longer wrongly blocks this legitimate case. VCF Automation — missing "Dedicated VLAN" checkbox: the data field allowing VCF Automation to be placed on its own dedicated VLAN ("VCF Automation Network") already existed in the engine (VCF 9.1), but no checkbox was exposed in the UI, making it impossible for the user to move VCF Automation off the Management VM Network. Added the "Dedicated VLAN (fleet lifecycle API)" checkbox in the VCF Automation block (visible only in 9.1) |
| v1.17.0 | Jul 2026 | Two fixes from an in-depth verification against techdocs.broadcom.com. Blocking fix: VCF Automation (9.1) was missing the "VCF Services Runtime (Automation)" appliance — a component distinct from the VIP endpoint, with its own dedicated FQDN and IP. Confirmed by two official Broadcom VCF 9.1 pages (Network Design Options + VCF Components FQDNs and IP addresses), which document a total of 2 FQDNs and 7 IPs for VCF Automation (1 VIP + 1 dedicated Services Runtime + 5 IPs node block), identical in Simple and High Availability mode. The app previously modeled only 1 FQDN and 6 IPs (5 nodes + a miscounted VIP); corrected to 7 IPs everywhere (Management VM Network VLAN sizing, dedicated VLAN, VCF Components panel) and added the missing appliance. Documentation fix: the "min 2 NFS hosts" help note previously cited VCF 5.2 documentation with a caveat ("9.x equivalent not located"). Now confirmed by an official VCF 9.1 page (Single-Rack vSphere Cluster Model): 2 hosts minimum with NFS/VMFS-FC storage in the "Simple" deployment model (4 hosts in "High Availability") — text updated accordingly in the 3 affected notes |
